    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-white font-sans px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-[400px]">
        ...
        <div className="text-center mb-8">
          <h1 className="text-[40px] md:text-[48px] font-bold text-apple-black tracking-[-0.02em] leading-tight mb-3">Create your account.</h1>
          <p className="text-[17px] text-apple-gray font-medium">Join the community to start swapping skills.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full py-4 bg-apple-black text-white rounded-[980px] text-[15px] font-medium hover:bg-[#333333] active:scale-[0.98] transition-all flex items-center justify-center min-h-[56px] mt-6 disabled:opacity-70 disabled:active:scale-100"
          >
            {loading ? (
        ...
        <p className="text-center text-[15px] text-apple-gray mt-10 font-medium">
          Already have an account? <Link to="/login" className="text-apple-black hover:underline ml-1">Log in →</Link>
        </p>
      </motion.div>
    </div>
  );
